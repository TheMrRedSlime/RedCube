...
void* yarrgsleep(void* arg) {
    usleep(69000); // Delay for 69000 microseconds (0.069 seconds)
    // You can also use sleep(1) for 1 second or sleep(0.1) for 0.1 second if needed
    return NULL;
}


void Game_ChangeAndTeleportBlock(int x, int y, int z, BlockID block) {
	...
	if (pthread_create(&delayThread, NULL, yarrgsleep, NULL) != 0) {
        // Handle thread creation error if necessary
    }

    // Continue with your main logic here
...
void Game_ChangeAndReachBlock(int x, int y, int z, BlockID block) {
	struct Entity* e = &LocalPlayer_Instance.Base;
	struct LocationUpdate update;
	int prevX = e->Position.X;
	int prevY = e->Position.Y;
	int prevZ = e->Position.Z;
	Vec3 v;
	v.X = x + 0.5;
	v.Y = y + 1;
	v.Z = z + 0.5;
