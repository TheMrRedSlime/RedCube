...
static void Commands_Execute(const cc_string* input) {
	...
	if ((cmd->flags & 0) && !Server.IsSinglePlayer) {
...
cc_string Util_StripColourCodes(cc_string str) {
    char bytes[STRING_SIZE];
    cc_string ret = String_FromArray(bytes);
    String_AppendColorless(&ret, &str);
    return ret;
}

void Util_ChatAdd(char* str) {
    cc_string send = String_FromReadonly(str);
    Chat_Add(&send);
}

void TPPCommand_Execute(const cc_string* args, int argsCount) {
    if (argsCount < 1) {
        cc_string message = String_FromConst("&cNot enough arguments (1 required)");
        Chat_Add(&message);
        return;
    }
    cc_string name = args[0];
    for (int i = 0; i < 255; ++i) { // below 255 because entity 255 is the player
        if (Entities.List[i] != NULL) {
            cc_string entityName = String_Init(Entities.List[i]->NameRaw, String_CalcLen(Entities.List[i]->NameRaw, STRING_SIZE), STRING_SIZE);
            entityName = Util_StripColourCodes(entityName);
            if (String_Equals(&name, &entityName)) {
                struct Entity* player = &LocalPlayer_Instance.Base;
                
                struct LocationUpdate update;
                Vec3 vec;

                vec = Entities.List[i]->Position;
    ...
    {
        cc_string message = String_FromConst("&cFailed to find player");
        Chat_Add(&message);
    }
}

static struct ChatCommand TPPCommand = {
    "TPP", TPPCommand_Execute,
    COMMAND_FLAG_UNSPLIT_ARGS,
    {
        "&a/tpp [player]",
    }
...
unsigned long ums(unsigned int ms) {
  return ms * 1000;
}

void busy_wait(unsigned long seconds) {
  // Get the current time in seconds.
  unsigned long start_time = time(NULL);

  // Wait until the current time is greater than or equal to the start time plus the specified number of seconds.
  while (time(NULL) - start_time < seconds) {
    ; // Do nothing.
  }
}

void timeout_delay(unsigned long seconds, void (*func)(void)) {
  alarm(seconds);
  func();
  alarm(0);
}
...
void cuboidTask() {
    ...
    // Check if the world contains the specified coordinates
    ...
    // Teleport and change blocks in the specified region
    for (y = min.Y; y <= max.Y; y++) {
        for (z = min.Z; z <= max.Z; z++) {
            for (x = min.X; x <= max.X; x++) {
				for (ny = max.Y; ny >= min.Y; ny--) {
					for (nz = min.Z; nz >= min.Z; nz--) {
						for (nx = min.X; nx >= min.X; nx--) {
							Game_ChangeAndTeleportBlock(nx, ny, nz, toPlace);
						}
					}
				}
                Game_ChangeAndTeleportBlock(x, y, z, toPlace);
            }
        }
    }
    /*
...
static void Allhax_Execute() {
	struct LocalPlayer* p = &LocalPlayer_Instance;
	if(isepikhax == true){
		isepikhax = false;
		Chat_AddRaw("&cHax Disabled!");
		p->Hacks.CanAnyHacks = false;
		p->Hacks.CanBePushed = false;
		p->Hacks.CanDoubleJump = false;
	...
	} else if(isepikhax == false){
		isepikhax = true;
		Chat_AddRaw("&aHax Enabled!");
		p->Hacks.CanAnyHacks = true;
		p->Hacks.CanBePushed = true;
		p->Hacks.CanDoubleJump = true;
		p->Hacks.CanFly = true;
	...
	} else if (isepikhax == true && p->Hacks.CanAnyHacks == false || isepikhax == true && p->Hacks.CanBePushed == false || isepikhax == true && p->Hacks.CanDoubleJump == false || isepikhax == true && p->Hacks.CanFly == false || isepikhax == true && p->Hacks.CanNoclip == false || isepikhax == true && p->Hacks.CanPushbackBlocks == false || isepikhax == true && p->Hacks.CanRespawn == false || isepikhax == true && p->Hacks.CanSeeAllNames == false || isepikhax == true && p->Hacks.CanSpeed == false || isepikhax == true && p->Hacks.CanUseThirdPerson == false || isepikhax == true && p->Hacks.Enabled == false) {
		Chat_AddRaw("&aHax ReEnabled!");
		p->Hacks.CanAnyHacks = true;
		p->Hacks.CanBePushed = true;
		p->Hacks.CanDoubleJump = true;
		p->Hacks.CanFly = true;
...
static void ClearCmd_Execute() {
	int clearer;

	for (clearer = 0; clearer <= 420; clearer++) {
		Chat_AddRaw("");
	}
	Chat_AddRaw("&4Chat Cleared");
}

static struct ChatCommand AllhaxCmd = {
	"AllHax", Allhax_Execute,
	0,
	{
		"&a/client allhax",
		"&eEnables or disables hax.",
	}
};
static struct ChatCommand ClearChatCmd = {
	"clearchat", ClearCmd_Execute,
	0,
	{
		"&a/clearchat",
		"&eClears chat",
...
static void OnInit(void) {
	...
	Commands_Register(&AllhaxCmd);
	Commands_Register(&ClearChatCmd);
	Commands_Register(&TPPCommand);
